...
class PS_Pagination {
	...
	function PS_Pagination($connection, $sql, $rows_per_page = 10, $links_per_page = 5, $append = "") {
		...
		$this->append = $append;
		$this->php_self = htmlspecialchars($_SERVER['REQUEST_URI'] );
		
		//Check if other page
		$str = substr(htmlspecialchars($_SERVER['REQUEST_URI'] ),-2);
		
		if(is_numeric(substr($str,0,1))){
			
			$str = substr(htmlspecialchars($_SERVER['REQUEST_URI'] ),0,-2);
			$this->php_self = $str;
		}
	...
	function renderFirst($tag = 'First') {
		...
		if ($this->page == 1) {
			...
			return '<a href="' . $this->php_self . '1/' . $this->append . '">' . $tag . '</a> ';
		}
	}
	...
	function renderFullNav() {
		return "<br><br>".$this->renderFirst() . '&nbsp;' . $this->renderPrev() . '&nbsp;' . $this->renderNav() . '&nbsp;' . $this->renderNext() . '&nbsp;' . $this->renderLast();
	}
